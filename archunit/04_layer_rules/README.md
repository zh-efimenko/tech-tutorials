...
`consideringAllDependencies()` — учитывает все виды зависимостей между слоями, а не только вызовы методов: поля, параметры и типы возвращаемых значений, наследование, аннотации.
...
> **Внимание:** каждый слой onion-модели должен содержать хотя бы один класс, иначе правило падает: `Layer 'web adapter' is empty`, `Layer 'domain model' is empty` и так далее — все пустые слои сразу. Имена в сообщении — роли в onion-модели, а не строки из `adapter("web", ..)`; кроме объявленных слоёв проверяется ещё и агрегатный `adapter` (все адаптеры вместе). Если структура совпадает не полностью, добавь `.withOptionalLayers(true)`.
...
Первое — "контроллеры не зависят от репозиториев". Второе — "репозитории используются только сервисами".

Направления кажутся зеркальными, но проверяют разное. `dependOnClassesThat` считает зависимостью всё: поле, параметр конструктора, тип возвращаемого значения, вызов метода. `onlyBeAccessed` учитывает только фактические обращения из байткода — объявление поля типа `BetRepository` в контроллере оно не поймает.

Отдельно подводят Spring Data-репозитории: вызов унаследованного `findAll()` в байткоде адресован тому интерфейсу, где метод объявлен (`ListCrudRepository`), а не твоему `BetRepository`. Правило `onlyBeAccessed` останется зелёным, хотя контроллер лезет в репозиторий напрямую; сработает оно только на методах, объявленных в самом интерфейсе (`findByUserId`).

> **Вывод:** для запрета доступа к слою бери `dependOnClassesThat` (или Layered Architecture API — он смотрит на зависимости), а `onlyBeAccessed` используй как дополнение, когда важны именно вызовы.
