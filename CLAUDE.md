...
Использовать skill `fix-md-escaping` — скрипт находит и чинит символы, которые рендерер читает как разметку (пара `$…$` съедает цены, `<tag>` в прозе исчезает, непарная `*` открывает курсив). Обязательно прогонять файлы, полученные конвертацией из HTML или PDF, до первого коммита.
